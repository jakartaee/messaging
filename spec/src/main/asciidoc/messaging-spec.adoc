...
support is beyond the scope of JMS, the design of Jakarta Messaging has always
...
Jakarta Messaging 1.0 defined two *domain-specific APIs*, one for point-to-point
...
Jakarta Messaging 1.1 introduced a new unified API which offered a single set of
interfaces that could be used for both point-to-point and pub/sub
messaging. This is referred to here as the *classic API*.

Jakarta Messaging 2.0 introduces a *simplified API* which offers all the features of
...
All interfaces are in the jakarta.Jakarta Messaging package.
...
“Restrictions on the use of Jakarta Messaging API in the Jakarta EE web or EJB
...
and “”. Jakarta Messaging providers and clients may provide more specialized
implementations.
...
New Jakarta Messaging defined properties may be added in later versions of JMS.
...
_<vendor_name>_. This is the mechanism a Jakarta Messaging provider uses to make its
...
message body directly. The Jakarta Messaging provider automatically creates a message
...
When the message has been successfully sent the Jakarta Messaging provider invokes the
...
its receipt to JMS. This ensures that Jakarta Messaging does not discard a partially
...
=== Restrictions on the use of Jakarta Messaging API in the Jakarta EE web or EJB container

Jakarta Messaging applications which run in the Jakarta EE web or EJB container are
...
* In a Jakarta EE web or EJB container, a Jakarta Messaging provider operates as a
...
specification. For more details see section 12.3 “Behaviour of Jakarta Messaging
...
=== Behaviour of Jakarta Messaging sessions in the Jakarta EE web or EJB container
...
Jakarta Messaging provider and which conforms to the Jakarta EE Connector Architecture
...
Jakarta Messaging sessions in the Jakarta EE web or EJB container” clarifies how the
...
Section 1.4.8 “Integration of Jakarta Messaging with the EJB components” has been
deleted. It is superseded by the new chapter 12 “Use of Jakarta Messaging API in Java
EE applications”.

==== Resource adapter (JMS_SPEC-25)

A new chapter 13 “Resource adapter” has been added which recommends, but
does not require, that a Jakarta Messaging provider (whether it forms part of a Java
EE application server or not) includes a resource adapter which connects
to that Jakarta Messaging provider and which conforms to the Jakarta EE Connector
...
Inflow to Jakarta Messaging Endpoints” in the Jakarta EE Connector Architecture
