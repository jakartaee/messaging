...
For more information on the use of JMS by Jakarta EE applications, see
chapter 12 “Use of JMS API in Jakarta EE applications”.
...
Jakarta EE web or EJB container. This information was previously only
available in the EJB and Jakarta EE platform specifications.
+
...
or asynchronously, except in a Jakarta EE web or EJB container where
...
“Restrictions on the use of JMS API in the Jakarta EE web or EJB
...
An asynchronous send is not permitted in a Jakarta EE EJB or web container.

The following methods must therefore not be used in a Jakarta EE web or EJB
container:
...
running in the Jakarta EE web or EJB container. This is recommended but not
required.
...
== Use of JMS API in Jakarta EE applications

=== Overview

The Java™ Platform, Enterprise Edition (Jakarta EE) Specification, v7
requires support for the JMS API as part of the full Jakarta EE platform.
...
the Jakarta EE 7 specification and the Enterprise JavaBeans 3.2
...
The JMS specification does not define how a Jakarta EE container integrates
with its JMS provider. Different Jakarta EE containers may integrate with
their JMS provider in different ways.

=== Restrictions on the use of JMS API in the Jakarta EE web or EJB container

JMS applications which run in the Jakarta EE web or EJB container are
...
* In a Jakarta EE web or EJB container, a JMS provider operates as a
...
sessions in the Jakarta EE web or EJB container”.
* The Jakarta EE web or EJB containers need to be able to manage the
threads used to run applications.
* The Jakarta EE web and EJB containers perform connection management which
...
their use by applications running in the Jakarta EE web or EJB container
...
called by applications running in the Jakarta EE web or EJB container:
...
be used by applications running in the Jakarta EE web or EJB container:
...
+
This restriction means that applications running in the Jakarta EE web or
...
An asynchronous send is not permitted in a Jakarta EE web or EJB container.
The following methods must therefore not be used in a Jakarta EE web or EJB
container:
...
=== Behaviour of JMS sessions in the Jakarta EE web or EJB container
...
which run in a Jakarta EE web or EJB container than it is for
applications +
which run in a normal Java SE environment or in the Jakarta EE
application +
client container.

When an application creates a Session or JMSContext in a Jakarta EE web +
...
* Applications running in a Jakarta EE web or EJB container are recommended
...
permitted in a Jakarta EE web or EJB container even if there is no active +
...
* In a Jakarta EE web or EJB container, when there is an active JTA
transaction in progress, session mode is ignored and the JMSSessionMode
annotation is unnecessary.
* In a Jakarta EE web or EJB container, when there is no active JTA
...
Jakarta EE web or EJB container” and the API documentation for the
ConnectionFactory method createContext(int sessionMode).
...
these methods are in any case prohibited in a Jakarta EE web or EJB
container.

== Resource adapter

The Jakarta EE Connector Architecture (JCA) specification defines a
standard architecture for connecting the Jakarta EE platform to enterprise
information systems (EISs).

A JMS provider (whether it forms part of a Jakarta EE application server or
not) is recommended to include a resource adapter which connects to that
JMS provider and which conforms to the Jakarta EE Connector Architecture
...
sending a TextMessage in a Jakarta EE web or EJB container.
...
This example compares the use of the classic and simplified JMS APIs for
synchronously receiving a TextMessage in a Jakarta EE web or EJB container.
...
in a Jakarta EE web or EJB container.
...
This example compares the use of the classic and simplified JMS APIs for
implementing a request/reply pattern in a Jakarta EE EJB container.
...
==== Use of JMS API in Jakarta EE applications (JMS_SPEC-45 and JMS_SPEC-27)

A new chapter 12 “Use of JMS API in Jakarta EE applications” has been
added. This chapter incorporates and clarifies various additional
requirements which were previously only described in the Jakarta EE and EJB
specifications. Section 12.2 “Restrictions on the use of JMS API in the
Jakarta EE web or EJB container” includes a list of methods which may not
be used in a Jakarta EE web or EJB container and section 12.3 “Behaviour of
JMS sessions in the Jakarta EE web or EJB container” clarifies how the
arguments to createSession are mostly ignored when used in a Jakarta EE web
or EJB container.
...
to that JMS provider and which conforms to the Jakarta EE Connector
...
Inflow to JMS Endpoints” in the Jakarta EE Connector Architecture
...
added. This has no arguments and is intended for use in a Jakarta EE web or
